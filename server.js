    ...
    try {
        ...
        if (!code || !planType) {
            return res.status(400).json({ 
                success: false, 
                accessAllowed: false, 
                error: "Activation code and target subscription plan are required." 
            });
        }

        const formattedCode = code.trim().toUpperCase();
        const requestedPlan = planType.trim().toLowerCase();

        // Check if code exists and was generated by backend
        const activeCode = await ActivationCode.findOne({ code: formattedCode });
        ...
        // Strict validation: Ensure user cannot use other subscription codes for subscription paid for
        if (activeCode.planType !== 'custom' && activeCode.planType !== requestedPlan) {
            return res.status(403).json({
                success: false,
                accessAllowed: false,
                error: `Subscription mismatch! Code belongs to '${activeCode.planType.toUpperCase()}' plan, but you requested '${requestedPlan.toUpperCase()}'.`
            });
        }
        ...
        return res.json({
            ...
            message: "Activation code verified successfully for this subscription plan. Access allowed.",
            codeDetails: {

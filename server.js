...
const activationCodeSchema = new mongoose.Schema({
    ...
    planType: { type: String, enum: ['starter', 'pro', 'ultimate', 'custom'], default: 'custom', required: true },
...
/**
 * 1. VERIFY & CHECK ACTIVATION CODE FROM CREATE PANEL PAGE
 * Validates backend-generation, verification state, and subscription match before allowing access to customer-dashboard.html
 */
app.post("/api/activation-codes/verify-access", auth, async (req, res) => {
    try {
        const rawCode = req.body.code || req.body.activationCode || req.body.key;
        const rawPlan = req.body.planType || req.body.plan || req.body.selectedPlan;
        ...
        // Flexible search matching exact case or case-insensitive fallback
        let activeCode = await ActivationCode.findOne({ code: formattedCode });
        if (!activeCode) {
            activeCode = await ActivationCode.findOne({ code: new RegExp("^" + formattedCode + "$", "i") });
        }
        ...
        if (!activeCode) {
            return res.status(404).json({ 
                success: false, 
                accessAllowed: false, 
                error: "Invalid activation code. Code was not generated by backend." 
            });
        }
        ...
        if (activeCode.status !== "active" || (activeCode.maxUses && activeCode.usedCount >= activeCode.maxUses)) {
            return res.status(400).json({ 
                success: false, 
                accessAllowed: false, 
                error: "Activation code has already been used or expired." 
            });
        }
        ...
        // Validate plan match if specified
        if (rawPlan) {
            const requestedPlan = String(rawPlan).trim().toLowerCase();
            if (activeCode.planType && activeCode.planType !== 'custom' && activeCode.planType.toLowerCase() !== requestedPlan) {
                return res.status(403).json({
                    success: false,
                    accessAllowed: false,
                    error: `Subscription mismatch! Code belongs to '${activeCode.planType.toUpperCase()}' plan, but '${requestedPlan.toUpperCase()}' was requested.`
                });
            }
        }
        ...
        return res.json({
            success: true,
            accessAllowed: true,
            redirectTo: "/customer-dashboard.html",
            message: "Activation code verified successfully. Access allowed.",
            codeDetails: {
                code: activeCode.code,
                planType: activeCode.planType,
                amount: activeCode.amount,
                serviceName: activeCode.serviceName,
                status: activeCode.status
            }
        });

    } catch (err) {
        console.error("Verify code access error:", err);
        res.status(500).json({ success: false, accessAllowed: false, error: "Server error during activation code verification." });
    }
});

/**
 * 2. REDEEM ACTIVATION CODE
 * Verifies and applies activation code directly to user account and redirects to customer-dashboard.html
 */
app.post("/api/activation-codes/redeem", auth, async (req, res) => {
    try {
        const { code, planType } = req.body;
        if (!code) return res.status(400).json({ success: false, error: "Activation code is required." });
        ...
        if (!activeCode) {
            return res.status(400).json({ success: false, error: "Invalid code. Activation code was not generated by backend." });
        }

        if (activeCode.status !== "active" || (activeCode.maxUses && activeCode.usedCount >= activeCode.maxUses)) {
            return res.status(400).json({ success: false, error: "Code has already been used or is expired." });
        }

        // Verify plan type alignment if specified
        if (planType) {
            const requestedPlan = String(planType).trim().toLowerCase();
            if (activeCode.planType && activeCode.planType !== 'custom' && activeCode.planType.toLowerCase() !== requestedPlan) {
                return res.status(403).json({
                    success: false,
                    error: `Subscription Mismatch! You cannot use a '${activeCode.planType}' code for '${requestedPlan}'.`
                });
            }
        }
        ...
        res.json({
            ...
            accessAllowed: true,
            redirectTo: "/customer-dashboard.html",
            message: `Successfully verified and redeemed code for ${activeCode.serviceName} (KES ${activeCode.amount})!`,
            newBalance: user.balance
        });
    } catch (err) {
        console.error("Redeem activation code error:", err);
        res.status(500).json({ success: false, error: "Failed to process activation code redemption." });
    }
});
...
app.post("/api/admin/activation-codes/generate", adminAuth, async (req, res) => {
    try {
        ...
        let resolvedPlan = planType ? planType.toLowerCase() : "custom";
        ...
        for (let i = 0; i < numToGenerate; i++) {
            ...
            if (code && numToGenerate === 1) {
                rawCode = String(code).trim().toUpperCase();
            } else {
